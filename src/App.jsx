...
export default function App() {
  ...
  useEffect(() => {
    ...
    supabase.removeChannel(supabase.channel(`notifs-${uid}`));
...
function ChatScreen({ friend, userId, onClose, onUnreadClear }) {
  ...
  useEffect(() => {
    ...
    supabase.from("conversations").select("id").eq("user_a",a).eq("user_b",b).maybeSingle()
      .then(async ({ data, error }) => {
        if (data) { setConvId(data.id); return; }
        const { data: nc, error: ie } = await supabase.from("conversations")
          .insert({user_a:a,user_b:b}).select("id").single();
        if (ie) console.error("Conv create error:", ie);
  ...
  useEffect(() => {
    ...
    const ch = supabase.channel(chName)
      .on("postgres_changes",{event:"INSERT",schema:"public",table:"messages",filter:`conversation_id=eq.${convId}`},
        ...
        if (status === "SUBSCRIBED") console.log("Chat subscribed to convId:", convId);
      });
    return () => supabase.removeChannel(ch);
  }, [convId, userId]);
  ...
  const send = async () => {
    ...
    const { error } = await supabase.from("messages").insert({conversation_id:convId,sender_id:userId,content});
    if (error) {
